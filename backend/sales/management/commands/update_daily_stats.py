...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if len(today_orders) > 2:
            ...
            OrderStats.objects.filter(location='Both').update(stats_json=convert_to_serializable(both_stats_dict))
            OrderStats.objects.filter(location='Bakery').update(stats_json=convert_to_serializable(bakery_stats_dict))
            OrderStats.objects.filter(location='Cafe').update(stats_json=convert_to_serializable(cafe_bakery_dict))
            OrderStats.objects.filter(location=f'Both_items_{item_name}').update(stats_json=convert_to_serializable(both_items_dict))
            OrderStats.objects.filter(location=f'Bakery_items_{item_name}').update(stats_json=convert_to_serializable(bakery_items_dict))
            OrderStats.objects.filter(location=f'Cafe_items_{item_name}').update(stats_json=convert_to_serializable(cafe_items_dict))
