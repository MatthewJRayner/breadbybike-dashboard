...
const ItemsDashboard = () => {
    ...
    useEffect(() => {
        const fetchCatalogItems = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL}/api/square-catalog-items/`);
    ...
    const triggerCalculation = async (loc, item) => {
        setLoading(true);
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/trigger-calculation/`, {
                method: 'POST',
    ...
    const fetchStats = async (locs) => {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/order-stats/?locations=${locs.join(',')}`);
        if (!response.ok) {
